...
class MyLED(object):
    ...
    def color_random(self, cycles, wait_time=0.001):
        for i in range(0, cycles):
            a = random.randrange(0, 100, 1)
            r = random.randrange(0, 0xFF, 1)
            g = random.randrange(0, 0xFF, 1)
            b = random.randrange(0, 0xFF, 1)
            self._strip[matrix[a]] = (r, g, b)
            self._strip.show()
            time.sleep(wait_time)

    def color_wipe(self, r, g, b, wait_time=0.05):
        for i in range(self._led_count):
            self._strip[matrix[self._led_count]] = (g, r, b)
            self._strip.show()
            time.sleep(wait_time)

    def scroll_text_display(self, string, color, wait_time):
    ...
    def print_char(self, char, cursor, r, g, b):
        for x in range(0, 3):
            var = font5x3[ord(char)][x]
            for y in range(0, self._led_height):
                if var & mask[y]:
                    self._strip[matrix[y * 20 + (cursor + x)]] = (g, r, b)
                else:
                    self._strip[matrix[y * 20 + (cursor + x)]] = (0, 0, 0)
                self._strip.show()
